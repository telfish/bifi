...
class LongBasedDomainMap[T, R, A: ClassManifest](val domain: RangeDomain[T, R], protected[bifi] val theMap: LongRangeMap[A]) extends DomainMap[T, R, A] {
  ...
  def getTokenByIndex(l: Long): Token = theMap.getToken(l)
  def isDefinedAtToken(token: Token): Boolean = theMap.isDefinedAtToken(token)
  def getByToken(token: Token): A = theMap.getByToken(token)
  ...
  def normalizeWithRange[B: ClassManifest](merge: (() => List[R], List[A]) => B): DomainMap[T, R, B] = {
    val normalized = theMap.normalizeWithRange { (start, end, list) =>
      merge(() => domain.rangeify(start, end + 1), list)
    } map {
      case (s, e, v) => Entry[B](s, e - s, v)
    }
    new LongBasedDomainMap(domain, RLELongRangeMap.fromSortedEntries[B](normalized.toIterable))
